...
which contains context and instructions to help the agent work on MQT SQR. For
...
a breaking change for MQT SQR. If you are sure that the dependency update does
not introduce any breaking changes for MQT SQR, you can remove the {code}`minor`
or {code}`major` label from the PR. This will ensure that the respective PR does
not influence the type of an upcoming release.
